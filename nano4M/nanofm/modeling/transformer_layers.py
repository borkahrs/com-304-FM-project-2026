...
class Mlp(nn.Module):
    ...
    def __init__(self, 
        ...
        # Apparently cleaner to write that way when single layer 
        # (not use Sequential)
...
class Attention(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, qkv_bias: bool = False, proj_bias: bool = False):
        ...
        # We can do a combined fc (K will take the first dim els, Q - the second, V - the third)
        self.kqv_fc = nn.Linear(dim, 3*dim, bias=qkv_bias)
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        kqv = self.kqv_fc(x) # shape: (B, L, 3*D)
        # Important: the shape operation reinterprets the last dimension! Doesn't move data around
        kqv = torch.reshape(kqv, (B, L, 3, self.num_heads, -1))
        kqv = torch.permute(kqv, (2, 0, 3, 1, 4)) # shape (3, B, num_heads, L, head_dim)
        q, k, v = torch.unbind(kqv, 0) # each of shape (B, num_heads, L, head_dim)
        ...
        attn = (q @ torch.transpose(k,-2,-1))*self.scale
        ...
        if mask is not None:
            ...
            attn = attn.masked_fill(~mask, torch.finfo(attn.dtype).min) # hypothesis: a more stable -inf representation

        # TODO: Compute the softmax over the last dimension
        attn = torch.nn.functional.softmax(attn, -1)

        # TODO: Weigh the values V by the attention matrix and concatenate the different attention heads
        # Make sure to reshape the output to the original shape of x, i.e. [B L D]
        x = attn @ v
        x = x.transpose(1, 2).contiguous().view(B, L, D)
        x = torch.reshape(x, (B, L, D))
...
class Block(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, mlp_ratio: float = 4., use_bias: bool = False):
        super().__init__()
        self.norm1 = LayerNorm(dim, bias=use_bias) # FIXME: most prob w/o bias
